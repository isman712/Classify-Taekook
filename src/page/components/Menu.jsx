...
function Menu() {
  ...
  const renderLists = (folderId) => {
    return lists.filter(list => list.folder_id === folderId).map((list) => (
      <li key={list.id}>
        <Link
          to={`/panel/${folderId}/${list.id}`}
          className={location.pathname === `/panel/${folderId}/${list.id}` ? "menu-active" : ""}
        >
          <svg
      ...
      <ul>
        {folderList.map((folder) => (
          <li key={folder.id}>
            <Link
              to={`/panel/${folder.id}`}
              className={location.pathname === `/panel/${folder.id}` ? "menu-active" : ""}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
  ...
  console.log(lists)
  return (
    <ul className="menu bg-base-200 w-80 h-full overflow-y-auto">
      {!folders ? (
        <div className="skeleton h-full w-80 border-none"></div>
      ) : folders.length === 0 ? (
